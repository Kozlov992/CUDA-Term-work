...
template <typename T>
T& CPUArray<T>::operator[] (size_t n) {	//for dim2 = 0, i.e. vectors
	return data[n];
};

template <typename T>
T CPUArray<T>::operator[] (size_t n) const {	//for dim2 = 0, i.e. vectors
	return data[n];
};

template <typename T>
T& CPUArray<T>::operator() (size_t n, size_t m) {
	return data[n * dim2 + m];
}

template <typename T>
T CPUArray<T>::operator() (size_t n, size_t m) const {
	return data[n * dim2 + m];
};

template <typename T>
CPUArray<T>::CPUArray(size_t d1, size_t d2) {
	dim1 = d1;
	dim2 = d2;
	if (dim2 == 0U)
		data = new T[dim1];
...
template <typename T>
CPUArray<T>::CPUArray(size_t d1, size_t d2, T* dta) {
	dim1 = d1;
	dim2 = d2;
	data = dta;
}
...
template <typename T>
size_t CPUArray<T>::getDim1() const {
	return dim1;
}

template <typename T>
size_t CPUArray<T>::getDim2() const {
	return dim2;
}

template <typename T>
T const* CPUArray<T>::getData() const {
	return data;
}

template <typename T>
void CPUArray<T>::add(const CPUArray& arr) {
	if (dim2 == 0) {
...
template <typename T>
void CPUArray<T>::subtract(const CPUArray& arr) {
	if (dim2 == 0) {
		for (int i = 0; i < dim1; i++) {
			data[i] -= arr[i];
		}
...
template <typename T>
void CPUArray<T>::setValue(T value) {
	if (dim2 == 0) {
		for (int i = 0; i < dim1; i++) {
			data[i] = value;
		}
...
template <typename T>
size_t GPUArray<T>::getDim1() const {
	return dim1;
}

template <typename T>
size_t GPUArray<T>::getDim2() const {
	return dim2;
}

template <typename T>
T const* GPUArray<T>::getData() const {
	return data;
}

template <typename T>
void GPUArray<T>::fill(const CPUArray<T>& arr) {
	auto fillerData = arr.getData();
	auto rc = cudaMemcpy(data, fillerData, size, cudaMemcpyHostToDevice);
}

template <typename T>
void GPUArray<T>::fill(T* arr) {
	auto rc = cudaMemcpy(data, arr, size, cudaMemcpyHostToDevice);
};

template <typename T>
void GPUArray<T>::print() const {
	if (dim2 == 0) {
		T* buf = new T[dim1];
...
template <typename T>
GPUArray<T>::GPUArray(const CPUArray<T>& arr) {
	dim1 = arr.getDim1();
	dim2 = arr.getDim2();
	auto fillerData = arr.getData();
	if (dim2 == 0U)
...
template <typename T>
GPUArray<T>::GPUArray(size_t d1, size_t d2, T* arr) {
	dim1 = d1;
	dim2 = d2;
	data = arr;
	if (dim2 == 0)
